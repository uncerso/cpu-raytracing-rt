...
fn raytrace_impl(ray: &Ray, scene: &Scene, rng: &mut ThreadRng, left_ray_depth: u8) -> RGB {
    ...
    return metadata.emission + match metadata.material {
        Material::Diffuse => (|| {
            ...
            let cosine_sampler = Cosine::new(intersection.shading_normal);
            let mix_sampler = Mix::new(Cosine::new(intersection.shading_normal), Light::new(intersection_pos, &scene.lights));
            ...
            if rng_dir.dot(intersection.shading_normal) <= 0.0 {
                return zero();
            }
            ...
            return rng_dir.dot(intersection.shading_normal) * metadata.color.mul_element_wise(light_from_dir) / PI / pdf;
        })(),
...
fn reflection_power(n1: Float, n2: Float, ray: &Ray, intersection: &Intersection) -> Float {
    ...
    r0 + (1.0 - r0) * (1.0 + ray.dir.dot(intersection.shading_normal)).pow(5)
}

fn reflected_ray(ray: &Ray, intersection: &Intersection) -> Ray {
    let dir = ray.dir - 2.0 * intersection.shading_normal * intersection.shading_normal.dot(ray.dir);
    Ray {
...
fn refracted_ray(ray: &Ray, intersection: &Intersection, reflection_index: Float) -> Option<Ray> {
    let cos_theta1 = -intersection.shading_normal.dot(ray.dir);
    ...
    let dir = reflection_index * ray.dir + (reflection_index * cos_theta1 - cos_theta2) * intersection.shading_normal;
    Some(Ray {
        origin: ray.position_at(intersection.t) + EPSILON * dir,
        dir,
    })
